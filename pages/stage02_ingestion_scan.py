...
# ---------------- LEFTOVER DISCOVERY ----------------
def collect_leftover_files(project_root: Path):
    """
    Finds folders named Leftovers_YYYY_MM_DD anywhere under project root
    and returns all contained image paths.
    """
    leftovers = []

    for folder in project_root.rglob("Leftovers_*"):
        if not folder.is_dir():
            continue

        for f in folder.rglob("*"):
            if f.suffix.lower() in (".png", ".jpg", ".jpeg"):
                leftovers.append(f.resolve())
...
def reconcile_db_with_moves(conn: sqlite3.Connection, input_dir: Path):
    ...
    disk_files = [f.resolve() for f in input_dir.rglob("*")
                  if f.suffix.lower() in (".png", ".jpg", ".jpeg")]
...
def process_images(input_dir: Path, conn: sqlite3.Connection,
                   existing_files=None, include_leftovers=False):
    ...
    if include_leftovers:
        leftovers = collect_leftover_files(input_dir.parent)
        if leftovers:
            roots.extend(set(p.parent for p in leftovers))
    ...
    for root in roots:
        ...
        imgs = [f for f in os.listdir(root)
                if f.lower().endswith((".png", ".jpg", ".jpeg"))]

        if not imgs:
            continue

        rel_path = Path(root).relative_to(input_dir.parent)
        ...
        sorted_images = [img_metadata[i] for i in np.argsort(avg_colors)]
        ...
        for i in range(0, len(sorted_images), batch_capacity):
            ...
            c.execute("""
                ...
                VALUES (?, ?, 'incomplete', ?, ?, ?, ?, ?)
            """, (primary_folder, secondary_folder, tile_w, tile_h,
                  batch_capacity, batch_name, ts))
...
st.write("Scans cleaned images and creates batches for layout & stitching.")

include_leftovers = st.toggle(
    "Include leftover images from previous batches",
    value=False,
    help="Pulls images from Leftovers_YYYY_MM_DD folders and merges into batches."
)
